...
function savePanic(value){
  ...
  document.getElementById("app").innerHTML = `<h1>Analyzing Threat...</h1>`;
  ...
  .then(async response => {
    // Hardened check: if the server crashes, throw the real error
    if (!response.ok) {
...
function requestPlaybook(os){
  document.getElementById("app").innerHTML = `<h1>Generating Playbook...</h1>`;
